...
const Working = () => {
  ...
  return (
    <motion.section
      ...
      variants={container}
      className="px-4 sm:px-6 lg:px-8 md:py-16 bg-white"
    >
      <div className="max-w-6xl mx-auto">
        ...
        {steps.map((step) => (
          ...
          >
            <motion.div
              variants={fadeUp}
              className={`${step.id % 2 === 0 && "order-1 md:order-2"}`}
            >
              ...
              <div className="space-y-4 mb-6 text-justify">
                {step.points.map((pt, i) => (
                  ...
                  >
                    <CircleDot
                      className={`w-4 h-4 flex-shrink-0 mt-1 ${pt.color}`}
                    />
                    <p className="text-[#858383] text-sm">
                      {pt.type && (
                        <span className={`font-semibold ${pt.color}`}>
                          {pt.type}:
                        </span>
                      )}{" "}
                      {pt.text}
